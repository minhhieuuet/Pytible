...
def findUser(senderFavorite, senderGender, senderId):
    ...
    if(senderFavorite == 'any'):
        result = list(usersCollection.find({"$or":[{"favorite": senderGender},{"favorite":"any"}],"_id":{"$ne":senderId}})) 
        if(len(result)==0):
            return senderId
        recId = senderId
        ...
        result = list(usersCollection.find({"$or":[{ "favorite": senderGender },{ "favorite": "any" }],"gender": senderFavorite, "_id":{"$ne":senderId}}))
        if(len(result)==0):
            return senderId
        recId = senderId
        while recId == senderId:
